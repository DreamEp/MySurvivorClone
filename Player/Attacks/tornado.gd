...
func _ready():
	...
	match last_movement: #Match ici le dernier mouvement de notre joueur
		Vector2.UP, Vector2.DOWN: #Si on se déplace vers la gauche ou la droite
			...
			move_to_less = global_position + Vector2(randf_range(-1, -0.25), last_movement.y) * 500 #On recupere la position ou la tornado spawn et on y ajoute 500 pixel dans la direction ou il va être envoyé ici a gauche
			move_to_more = global_position + Vector2(randf_range(0.25, 1), last_movement.y) * 500 #On recupere la position ou la tornado spawn et on y ajoute 500 pixel dans la direction ou il va être envoyé ici a droite
		Vector2.RIGHT, Vector2.LEFT: #Si on se déplace vers le haut ou le bas on fait varier que l'axe des y
			move_to_less = global_position + Vector2(last_movement.x, randf_range(-1, -0.25)) * 500 #On recupere la position ou la tornado spawn et on y ajoute 500 pixel dans la direction ou il va être envoyé ici a gauche
			move_to_more = global_position + Vector2(last_movement.x, randf_range(0.25, 1)) * 500 #On recupere la position ou la tornado spawn et on y ajoute 500 pixel dans la direction ou il va être envoyé ici a droite
